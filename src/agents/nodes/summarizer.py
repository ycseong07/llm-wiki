...
목표:
1) **요약이 아니라 "정리본"** 을 작성. 한국어 마크다운으로:
   - 첫 단락: 핵심 주장/결론을 2~4문장으로 정리
   - 그 다음 불릿 리스트: 본문의 중요한 사실을 빠짐없이 추출 (숫자, 인명, 회사명, 제품/기술명, 인용, 데이터, 논리 흐름, 비교/대조)
   - 분량 상한 없음. 본문이 길면 정리본도 길어야 한다. 핵심 정보 손실 금지
   - 본문에 없는 내용 추측·보충 금지. 자료 안에서만 추출
   - 참조 글에만 있는 디테일도 동등하게 활용
2) 콘텐츠 전체를 대표하는 키워드/엔티티 3~5개 (고유명사·기술용어·회사명·인명 우선). 한·영 혼용 가능

JSON으로만 응답 (다른 텍스트 금지). summary 값은 줄바꿈 포함 마크다운 문자열:
{{"summary": "...정리본 마크다운...", "tags": ["키워드1", "키워드2", "키워드3"]}}
